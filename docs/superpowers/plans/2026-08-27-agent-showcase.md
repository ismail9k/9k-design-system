...
**Background the implementer needs:** Almost every component in `src/components/` uses the same declaration shape — `withDefaults(defineProps<{ … }>(), { … })` inside `<script setup lang="ts">`. A few use a bare `defineProps<{ … }>()` with no defaults. **One does neither:** `I9kBrandWordmark.vue` uses the runtime form, `defineProps({ compact: { type: Boolean, default: false }, full: { type: String, default: 'Ismail9k' }, short: { type: String, default: '9k' } })`. Handle all three, and never return an empty prop list for a component that actually declares props — a silent `props: []` is the exact drift this extractor exists to prevent. (`I9kBlurredCircles.vue` has no `<script>` block and genuinely has no props; `props: []` is correct there.) Prop types are either inline (`string | Record<string, unknown> | null`), a locally declared alias (`type Variant = 'default' | 'primary' | …` in `I9kButton.vue`), or an alias imported from `src/types/` (`I9kComponentSize`).
